...
public partial class PatternBlockSpawner : Node2D
{
    ...
    private void InitializeBlockTemplates()
    {
        ...
        {
            (DraggableShape.ShapeType.Persegi, new Color(1f, 0.2f, 0.2f), baseSize),           // Red Square
            (DraggableShape.ShapeType.SegitigaSamaKaki, new Color(0.2f, 1f, 0.2f), baseSize), // Green Triangle
            (DraggableShape.ShapeType.Hexagon, new Color(0.2f, 0.2f, 1f), baseSize),          // Blue Hexagon
            (DraggableShape.ShapeType.JajarGenjang, new Color(1f, 1f, 0.2f), baseSize),       // Yellow Parallelogram
            (DraggableShape.ShapeType.TrapesiumSiku, new Color(1f, 0.2f, 1f), baseSize),      // Magenta Trapezoid
            (DraggableShape.ShapeType.SegitigaSiku, new Color(0.2f, 1f, 1f), baseSize),       // Cyan Right Triangle
            (DraggableShape.ShapeType.Lingkaran, new Color(1f, 0.5f, 0.2f), baseSize),        // Orange Circle
        };
    }
    ...
    public DraggableShape SpawnBlock(DraggableShape.ShapeType type)
    {
        ...
        // IMPORTANT: Add to scene FIRST before connecting signals
        shapesContainer.AddChild(newBlock);

        spawnPosition += new Vector2(spacing, 0);

        // Emit signal yang akan di-handle oleh ChallengeLevel
    ...
    private void OnBlockSnapped(DraggableShape block)
    {
        GD.Print($"Block {block.Type} snapped to outline");
    }

    private void OnBlockDeleted(DraggableShape block)
    {
        GD.Print($"Block {block.Type} deleted");
    }
